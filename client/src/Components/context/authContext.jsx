...
//         })
//         .then((response) => {
//           setUser(response.data); // Set the user data if successful
//           fetchNotificationPreference(token); // Fetch notification preference on user load
//         })
//         .catch(() => setUser(null)); // If the token is invalid or expired, log out and set user to null
//     }
//   }, []);
...
//         'http://localhost:5001/api/user/notifications/toggle',
...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (token) {
      axios
        .get('http://localhost:5001/api/auth/me', { headers: { Authorization: `Bearer ${token}` } })
        .then((response) => {
